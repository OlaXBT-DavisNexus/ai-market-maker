...
def main() -> None:
    ...
    # — Load trades from CSV (preferred) or fallback to JSONL —
    ...
    # — Load iteration stances by bar_index —
    ...
    for it in _read_jsonl(iter_path):
        ...
        # data_quality per bar
        dq = it.get("data_quality") if isinstance(it.get("data_quality"), dict) else {}
        stances_by_bar[int(bi)] = {
            "entry_stance_action": str(ti.get("action", "HOLD")),
            "entry_stance_confidence": ti.get("confidence", 0.0),
            "entry_stance": str(ti.get("stance", "")),
            "data_quality_passed": dq.get("passed", True),
        }

    # — Annotate each trade with stance at its entry bar —
    ...
    for t in trades:
        ...
        hold_bars = int(t.get("holding_bars", 0)) if str(t.get("holding_bars", "")).isdigit() else None
    ...
    if hold_bars_all:
        ...
        print(f"Avg holding bars: {avg_hold:.1f}  (max: {max(hold_bars_all)}, min: {min(hold_bars_all)})")
    print(f"\nExit reason breakdown:")
    for reason, count in reason_ct.most_common():
        reason_pnls = pnl_by_reason[reason]
        avg_pnl = sum(reason_pnls) / len(reason_pnls)
        print(f"  {reason:20s}  count={count:4d}  avg_pnl={avg_pnl:+.2f}  total_pnl={sum(reason_pnls):+.2f}")
    ...
    if args.full:
        print(f"\n{'=' * 100}")
        print(f"{'ID':>4} | {'Side':6s} | {'Entry':>10s} | {'Exit':>10s} | "
        ...
        for r in annotated:
            ...
            print(f"{r['trade_id']:>4d} | {side_str:6s} | {r['entry_price']:>10s} | "
    ...
    if args.csv:
        csv_fields = [
            ...
            "pnl_usd", "holding_bars", "exit_reason", "entry_bar_index",
            "entry_stance_action", "entry_stance_confidence", "entry_stance",
        ]
