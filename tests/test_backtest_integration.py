...
class TestBacktestExportBundle:
    def test_3_bar_backtest_bundle(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            decision_lines = [l for l in lines if l.get("event_type") == "decision"]
            ...
            assert actions, "No decision.action found — audit ledger rows missing nested decision dict"
            # Verify bar_index present
            bar_indices = {d.get("bar_index") for d in decision_lines}
            assert all(bi is not None for bi in bar_indices), "bar_index missing in some audit entries"
