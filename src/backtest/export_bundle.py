...
def write_trades_csv(trades: list[dict[str, Any]], path: Path) -> None:
    ...
    fields = [
        "run_id", "trade_id", "symbol", "side",
        "entry_ts_ms", "exit_ts_ms",
        "entry_bar_index", "exit_bar_index",
        "entry_price", "exit_price",
        "size", "leverage",
        "pnl_usd", "pnl_pct", "commission_usd",
...
def write_equity_csv(snapshots: list[dict[str, Any]], path: Path) -> None:
    ...
    fields = [
        "bar_index", "ts_ms", "ts_utc",
        "capital", "unrealized_pnl", "equity", "positions", "drawdown_pct",
    ]
    peak = float("-inf")
...
def write_audit_ledger(
    ...
    *,
    ...
    # Decisions from iterations.jsonl
    ...
    # Sort: primary bar_index, secondary seq, fallback ts_ms
    rows.sort(key=lambda r: (r["bar_index"], r["seq"], r["ts_ms"]))

    # Write each row with ISO ts_utc enriched
...
def write_export_manifest(
    ...
    *,
    ...
    manifest = {
        ...
        "arbitrator_mode": summary.get("arbitrator_mode", summary.get("resolved_config", {}).get("arbitrator_mode", "")),
        "interval_sec": summary.get("interval_sec", summary.get("resolved_config", {}).get("interval_sec", 300)),
